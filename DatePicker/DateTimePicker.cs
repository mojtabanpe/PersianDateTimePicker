...
namespace DatePicker
{
    public partial class DateTimePicker : UserControl
    {
        ...
        private void FillGridDays(int year, int month)
        {
            ...
            {
                dgv_Calenar.Rows[rowIndex].Cells[cellIndex].Tag = new DateTime(month == 1 ? year - 1 : year, month == 1 ? 12 : month - 1, dayIndex, calendar);
            ...
            for (int dayIndex = 1; dayIndex <= currentMonthDays; dayIndex++)
            {
                ...
                if (currentCell.Year == SelectedDate.Year && currentCell.Month == SelectedDate.Month && currentCell.Day == SelectedDate.Day)
                {
                    OnSelectedDateChanged?.Invoke(this, new EventArgs());
            ...
            if (cellIndex > 0)
            {
                ...
                {
                    dgv_Calenar.Rows[rowIndex].Cells[cellIndex].Tag = new DateTime(month == 12 ? year + 1 : year, month == 12 ? 1 : month + 1, dayIndex, calendar);
        ...
        {
            FillGridDays(_currentMonth == 12 ? _currentYear + 1 : _currentYear, _currentMonth == 12 ? 1 : _currentMonth + 1);
        }
        private void btn_LastMonth_Click(object sender, EventArgs e)
        {
            FillGridDays(_currentMonth == 1 ? _currentYear - 1 : _currentYear, _currentMonth == 1 ? 12 : _currentMonth - 1);
        }
